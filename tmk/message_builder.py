...
За 1 день и за 15 минут до консультации придёт напоминание в этот чат."""
...
За 15 минут до консультации придёт напоминание в этот чат и ссылка на чат телемед консультации в случае вашего согласия."""
...
За 15 минут до консультации придёт напоминание в этот чат и ссылка на чат телемед консультации."""
...
def build_reminder_15m_with_link(session: Dict[str, Any]) -> str:
    """
    Напоминание за 15 минут со ссылкой на чат (только если есть согласие)
    ...
    message = f"""Уважаемый {patient_name}, напоминаем вам, что через 15 минут начнётся телемедицинская консультация.
...
def build_reminder_15m_without_consent(session: Dict[str, Any]) -> str:
    """
    Напоминание за 15 минут без ссылки (согласия нет)
...
def build_late_consent_message(session: Dict[str, Any]) -> str:
    """
    Сообщение при позднем согласии (после 15-минутного напоминания)
